...
import IFrameCard from '../../components/v3/IFrame/IFrameCard'
// import RowComponent from '../components/Row/Row'
import {useOnMount} from '../../hooks'
import {getTemplateVersion} from '../../features/v3/Template/templateSlice'
...
const ConceptTemplateVersionLayout: React.FC = () => {
  ...
  const imageVideoFiles: any = undefined
  ...
  const {templateVersion, isTemplateVersionSuccess} = useAppSelector((state) => state.template)
  ...
  useOnMount(() => {
    setLoading(!loading)
    dispatch(getTemplateVersion(params._id))
  })
  // useEffect(() => {
  //   setLoading(!loading)
        ...
        <Space.Compact block>
          <TreeSelectStyled
            ...
            onSearch={(value) => {
              ...
              setSelectAllVariantNames(false)
              let treeDataVariantNames: TreeNodeData[] = []
              templateVersion.map((_templateVersion) => {
                _templateVersion.variants.map((variantName: any) => {
                  const variantNameExist = treeDataVariantNames.some(
                    (el) => el.value === variantName.variantName,
                  )
                  if (!variantNameExist)
